...
def settings(request):
    """
    Adds the settings specified in settings.VISIBLE_SETTINGS_IN_TEMPLATES to
    the request context.
    """
    new_settings = {}
    for attr in getattr(django_settings, "VISIBLE_SETTINGS_IN_TEMPLATES", []):
        try:
            new_settings[attr] = getattr(django_settings, attr)
        except AttributeError:
            m = "VISIBLE_SETTINGS_IN_TEMPLATES: '{0}' does not exist".format(attr)
